<?php
	...
	{
		$query ="SELECT * FROM buyer WHERE status='active'";
		$cms = get($query);
		return $cms;	
    }
...
//		return $user[0];
  //  }
    ...
    function editUser($id, $name, $dob, $age, $mobile, $address, $email, $gender, $image)
    {
        $status= "active";
        $query="UPDATE customer SET name='$name', dob='$dob', age=$age, mobile='$mobile', address='$address', email='$email', gender='$gender', image='$image'  WHERE c_id='$id'";
		
		execute($query);
		header("Location:../../views/User/profile.php");
	}
	
	function insertUser($c_id, $name, $dob, $age, $mobile, $address, $email, $gender, $image, $status)
	{
		
		$query="INSERT INTO customer VALUES('$c_id','$name','$dob','$age','$mobile','$address', '$email','$gender','$image','$status')";
		execute($query);
